...
from src.transport.sonicsync_udp import SonicSyncUDPBroadcaster
...
class SourceManager:
    ...
    def switch_source(self, source_type: str):
        ...
        if source_type == "vlc":
            ...
            logger.warning(f"Unknown source type {source_type}, defaulting to test generator")
            self._active_source = self.test_source
...
class SonicSyncServer:
    ...
    def __init__(
        ...
        # Transport adapters
        self.udp_broadcaster = SonicSyncUDPBroadcaster(port=5004)
        self.rtp_adapter = RTPAdapter(self.audio_format)
        self.rtcp_adapter = RTCPAdapter(self.rtp_adapter.ssrc)
        self.ws_manager = WebSocketStreamManager(self.sync_coordinator)
    ...
    async def _audio_broadcast_loop(self):
        ...
        while self._is_running:
            ...
            # 2. RTP packet for VLC
            rtp_pkt = self.rtp_adapter.packetize(limited_audio, SampleFormat.INT16)
            rtp_bytes = rtp_pkt.serialize()
            self.udp_broadcaster.send_raw(rtp_bytes, target_ip="239.255.0.1", target_port=5004)
    ...
    async def _rtcp_report_loop(self):
        ...
        while self._is_running:
            ...
            try:
                sr = self.rtcp_adapter.create_sender_report(self.rtp_adapter._rtp_timestamp)
                sr_bytes = sr.serialize()
                self.udp_broadcaster.send_raw(sr_bytes, target_ip="239.255.0.1", target_port=5005)
